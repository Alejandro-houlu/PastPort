...
class DocumentProcessor:
    ...
    def process_and_add_documents(self, vdb, rootfolder: str, speciesfolder: str) -> int:
        """
        Process and add documents to the vector database.
        
        Args:
            vdb: VectorDBManager instance with initialized vdb
            rootfolder: Root folder containing species data
            speciesfolder: Species folder name to process
            
        Returns:
            Number of documents processed
        """
        chunks = self.load_chunks(rootfolder, speciesfolder)
        if chunks:
            # Add chunks to the vector database
            vdb.vdb.add_documents(chunks)
            return len(chunks)
        return 0
